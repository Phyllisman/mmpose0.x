...
class TopDownGenerateSimC:
    ...
    def _adjust_target_weight(self, cfg, joint, target_weight, tmp_size):
    ...
    def _generate_sa_simdr(self, cfg, joints, joints_vis):
        ...
        target_x = np.zeros((num_joints,
                            int(image_size[0]*2)),
                            dtype=np.float32)   # (17, 384)
        target_y = np.zeros((num_joints,
                            int(image_size[1]*2)),
                            dtype=np.float32)   # (17, 512)                           
        ...
        for joint_id in range(num_joints):
            target_weight[joint_id] = \
                self._adjust_target_weight(cfg, joints[joint_id], target_weight[joint_id], tmp_size)
            if target_weight[joint_id] == 0:
                continue
            ...
            x = np.arange(0, int(image_size[0]*2), 1, np.float32)
            y = np.arange(0, int(image_size[1]*2), 1, np.float32)
    ...
    def __call__(self, results):
        ...
        target_x, target_y, target_xy_weight  = self._generate_sa_simdr(results['ann_info'], joints_3d,joints_3d_visible)
